...
const ITHEUM_TOKEN_DEVNET = 'iTH89rapYE2GG4B77UjoEqnY7bRAPQrUs6LRAXQEHUo'
...
const PRIVATE_KEY_STR = process.env.ANCHOR_WALLET
...
const initializeContract = async () => {
  const transaction = await program.methods
    .initializeContract(
      1,
      new anchor.BN(43200), // Lock period (half a day)
      new anchor.BN(1000e9), // Example parameter
      new anchor.BN(1e4), // Example parameter
      new anchor.BN(8000), // Max APR
      new anchor.BN(8000) // Withdraw penalty
    )
    .signers([admin])
    .accounts({
...
const initializeVault = async () => {
  const vault_ata = await getOrCreateAssociatedTokenAccount(
    connection,
    admin,
    new PublicKey(ITHEUM_TOKEN_DEVNET),
    vaultConfig,
    true,
    'finalized'
  )
  ...
  const tx = await program.methods
    ...
    .accounts({
      ...
      mintOfToken: new PublicKey(ITHEUM_TOKEN_DEVNET),
...
// addRewards(new anchor.BN(5000000e9))
...
const bond = async () => {
  ...
  const transaction = await program.methods
    ...
    .accounts({
      ...
      vaultConfig: vaultConfig,
      vault: vault_ata,
      mintOfTokenSent: new PublicKey(ITHEUM_TOKEN_DEVNET),
      authority: user.publicKey,
